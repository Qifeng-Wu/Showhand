...
public class JwMediaAPI {
	...
	public static void main(String[] args){
		...
		/*	deleteArticlesByMaterial("lf43aKmvRzMeClhdDbeYLOLX3gI8G_2jCchvV-ekj7Ih6PdCKEMLs65c30dAnIgbRvy9_jA1ZD4r9E-ehBgD6MxTVSy-FXcI_MQP7S5x1Y4odEXdkDHPTXnw6Xk9sUS9DDMbALDTTC",
					"QWGYfLPfrB6VrQ9V5YdkdYM7mpv08Hc3C5QksQRx8Yk");*/
			
			WxDwonload d = JwMediaAPI.downMedia(
					"KhmnVS8jkwokUruK0KR1cGIlwgLdtstWNdmJbzklUzWnEAHqpfFlcYenKZuPdohws6t_Sa3C1SvxbtGF4t07lCvz_xdlWu41JDkTKm-Ca1bRb8DCPqsmua0_JJES2yUFQZOdAFDUDS",
					"QWGYfLPfrB6VrQ9V5YdkdQLvkV-zi32BJb7tjWltpdU",
					"C:/");
	...
	public static void deleteArticlesByMaterial(String accesstoken,String mediaId) throws WexinReqException {
		material_get_material_url="https://api.weixin.qq.com/cgi-bin/material/del_material?access_token=ACCESS_TOKEN";	
		if (accesstoken != null&&StringUtils.isNotEmpty(mediaId)) {
				String requestUrl = material_get_material_url.replace("ACCESS_TOKEN", accesstoken);
				WxArticlesRequestByMaterial wxArticlesRequestByMaterial = new WxArticlesRequestByMaterial();
				wxArticlesRequestByMaterial.setMediaId(mediaId);
				JSONObject obj = JSONObject.fromObject(wxArticlesRequestByMaterial);				
				String objs=obj.toString().replace("mediaId", "media_id");				
				
				JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST", objs);
				//System.out.println("微信返回的结果：" + result.toString());
				if (result.has("errcode")&&result.get("errcode")!="0") {
				if((result.get("errcode")+"").length()!=1&&Integer.parseInt((result.get("errcode")+""))!=0){	
	 ...
	 * @throws WexinReqException
	 */
	public static JSONObject queryArticlesByMaterial1(String accesstoken,String type,int offset,int count) throws WexinReqException {
		WxNews wn = null;
		if (accesstoken != null) {
			String requestUrl = material_batchget_material_url.replace("ACCESS_TOKEN", accesstoken);
